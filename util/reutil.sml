...
struct
  ...
  fun compile x =
      R.compileString x
          handle RegExpSyntax.CannotParse => raise REUtil "ill-formed regular expression"
               | RegExpSyntax.CannotCompile => raise REUtil "couldn't compile RE"
               | _ => raise REUtil "(unknown other error compiling regexp)"
        ...
        | SOME (tree, _) =>
            let
              fun g i =
                  ...
                  end handle Subscript => raise REUtil ("match " ^ Int.toString i ^ 
                                                        "  out of bounds")
            in
              SOME g
            end
